const API_BASE = (typeof process !== 'undefined' && process.env?.NEXT_PUBLIC_API_BASE) ? process.env.NEXT_PUBLIC_API_BASE : '/api';
...
async function request(path, options = {}) {
  ...
  if (!res.ok) {
    ...
    if ((res.status === 401 || res.status === 403) && !noRedirect && typeof window !== 'undefined') {
...
export async function openProofOfDelivery(proofPath) {
  ...
  if (!res.ok) {
    if ((res.status === 401 || res.status === 403) && typeof window !== 'undefined') window.location.href = '/login';
...
export const api = {
  suppliers: {
    ...
    delete: (id) =>
      fetch(`${API_BASE}/suppliers/${id}`, { method: 'DELETE', credentials: 'include' }).then((r) => {
  ...
  items: {
    ...
    delete: (id) => fetch(`${API_BASE}/items/${id}`, { method: 'DELETE', credentials: 'include' }).then((r) => { if (!r.ok) return r.json().then((d) => { throw new Error(d?.error); }); }),
  ...
  users: {
    ...
    delete: (id) => fetch(`${API_BASE}/users/${id}`, { method: 'DELETE', credentials: 'include' }).then((r) => { if (!r.ok) return r.json().then((d) => { throw new Error(d?.error); }); }),
  },
};
